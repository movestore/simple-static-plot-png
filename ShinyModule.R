...
shinyModule <- function(input, output, session, data,num=2) {
  ...
  ggsave(speed.plot, file = paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"),"speedhist.png"))
  ggsave(speed.plot, file = paste0(Sys.getenv(x = "APP_ARTIFACTS_DIR", "/tmp/"),"speedhist.pdf"))
